...
async function printUserStats (username) {
  const { _id: user_id } = await users.findOne({ username });
  const tossupCount = await tossupData.countDocuments({ user_id });
  const bonusCount = await bonusData.countDocuments({ user_id });
